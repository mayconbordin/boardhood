...
class StringField(Field):
    ...
    def validate(self, field, value):
        ...
        if self.minlength and len(value) < self.minlength:
            self.add_error(field, 'below_min_length', '%s is the minimum length' % str(self.minlength))
            return False

        if self.maxlength and len(value) > self.maxlength:
            self.add_error(field, 'above_max_length', '%s is the maximum length' % str(self.maxlength))
            return False
...
class DateField(StringField):
    ...
    def validate(self, field, value):
        value = super(DateField, self).validate(field, value)
        if value is False:
            return False
        ...
        try:
            ...
            self.add_error(field, 'invalid_format', 'valid format is %s' % self.format)
            return False
...
class NumField(Field):
    ...
    def validate(self, field, value):
        ...
        if self.min and num_value < self.min:
            self.add_error(field, 'below_min_value', '%s is the minimum value' % str(self.min))
            return False

        if self.max and num_value > self.max:
            self.add_error(field, 'above_max_value', '%s is the maximum value' % str(self.max))
            return False
...
class FloatField(NumField):
    ...
    def validate(self, field, value):
        value = super(FloatField, self).validate(field, value)
        if value is False:
            return False
